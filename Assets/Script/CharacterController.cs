...
public class CharacterController : MonoBehaviour
{
    ...
    public float launchForce = 10f; //lực nhảy va chạm kẻ thù
    ...
    public Button attackButton1; // Nút tấn công 1
    public Button attackButton2; // Nút tấn công 2
    ...
    void Update()
    {
        ...
        // Cập nhật vận tốc của Rigidbody2D dựa trên đầu vào ngang
        rb.velocity = new Vector2(horizontalInput * speed, rb.velocity.y);
        // Đây là phần di chuyển sẽ được điều khiển bởi các nút trên giao diện di động
        ...
        // Hoạt ảnh chạy
        isRunning = Mathf.Abs(rb.velocity.x) > 0; // Kiểm tra xem có di chuyển hay không
        anim.SetBool("isRunning", isRunning); // Cập nhật trạng thái chạy

        // Hướng di chuyển
        if (rb.velocity.x > 0 && !isFacingR)
        {
            Flip(); // Đảo hướng khi di chuyển sang phải
        }
        else if (rb.velocity.x < 0 && isFacingR)
        {
            Flip(); // Đảo hướng khi di chuyển sang trái
        }
    ...
    public void Attack2()
    {
        if (attack2Timer <= 0)
        {
            ...
            if (!isFacingR)
            {
                aDepLao.GetComponent<Rigidbody2D>().velocity = -transform.right * 10f;
            }
            else
            {
                aDepLao.GetComponent<Rigidbody2D>().velocity = transform.right * 10f;
            }
            audioManager.PlaySFX(audioManager.throwSlippers);
            Destroy(aDepLao, 2f);
        }
    }
    ...
    void ResetAttack()
    {
        anim.SetBool("isAttacking", false); // Reset trạng thái tấn công
    }

    void ResetAttack2()
    {
        anim.SetBool("isAttacking2", false); // Reset trạng thái tấn công thứ hai
    }
    ...
    void UpdateFallingState()
    {
        if (rb.velocity.y < 0)
        {
            anim.SetBool("isFalling", true); // Cập nhật trạng thái rơi khi di chuyển xuống dưới
        }
        else
        {
            anim.SetBool("isFalling", false); // Đặt lại trạng thái rơi khi không di chuyển xuống dưới
        }
    }
